...
public class GOALMentalStateConverter implements Serializable {
	...
	public GOALState translate(Set<DatabaseFormula> beliefs,
		...
		try {
			...
			for (DatabaseFormula formula : formulas) {
				...
				index = universe.getIndex(belief);
				if (index == -1) {
					index = universe.addIfNotContains(belief);
				}
				q.set(index);
			}
			...
			// Convert goals.
			int depth = 0;
			if (goalBaseStack.size() > goalBases.size()) {
				goalBases.add(goalBaseStack.peek());
			}
			for (GoalBase goalBase : goalBaseStack) {
				...
				for (SingleGoal singleGoal : goalBase) {
					...
					index = universe.getIndex(goal);
					if (index == -1) {
						index = universe.addIfNotContains(goal);
					}
					q.set(index);
				}
			...
			for (GoalBase module : goalBaseStack) {
				GOALCE_FocusAtDepth focus = new GOALCE_FocusAtDepth(module,
						depth);
				index = universe.getIndex(focus);
				if (index == -1) {
					index = universe.addIfNotContains(focus);
				}
				q.set(index);
		...
		try {
			...
			while (index++ < maxIndex) {
				...
				if (msBitSet.get(index) && !q.get(index)) {
					GOALConversionElement element = universe.getAtIndex(index);

					/* Belief */
					if (element instanceof GOALCE_Belief) {
						mentalState.delete(((GOALCE_Belief) element).belief,
				...
				if (!msBitSet.get(index) && q.get(index)) {
					...
					/* Belief */
					if (element instanceof GOALCE_Belief) {
						mentalState.getOwnBase(BASETYPE.BELIEFBASE).insert(
								((GOALCE_Belief) element).belief, debugger);
					}
	...
	public String toString(int indent) {
		...
		try {
			...
			AgentId name = mentalState.getAgentId();
			LinkedHashSet<DatabaseFormula> formulas = new LinkedHashSet<DatabaseFormula>();
			for (DatabaseFormula formula : mentalState
					.getOwnBase(BASETYPE.BELIEFBASE).getTheory().getFormulas()) {
