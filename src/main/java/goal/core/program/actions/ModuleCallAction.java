...
public class ModuleCallAction extends Action {
	...
	private static final long serialVersionUID = -6731655083707717118L;
	/**
	 * The parameters of this {@link ModuleCallAction}, i.e. the parameters of
	 * the module to be entered.
	 */
	...
	/**
	 * The substitution with which the module is to be instantiated. Is
	 * {@code null} until {@link #applySubst(Substitution)} has been called.
	 */
	private Substitution substitutionToPassOnToModule = null;
	/**
	 * The rule in which this action is used. Necessary to get the
	 * {@link FocusMethod} used, and if necessary the links between
	 * {@link Substitution}s and {@link SingleGoal}s. This is needed also when
	 * the {@link FocusMethod#FILTER} focus action is applied, to copy the goals
	 * there into our goal base.
	 */
	 ...
	 * @param parentRule
	 *            The {@link Rule} this action is defined in. Should only be
	 *            {@code null} when created to focus on the event module.
	 * @param arguments
	 *            The arguments of the action, used to instantiate the
	 *            parameters of the target module.
	 * @param source
	 *            Where the new action as defined.
	 */
	public ModuleCallAction(Module targetModule, Rule parentRule,
			List<Term> arguments, InputStreamPosition source) {
		super(targetModule.getName(), source);
	...
	/**
	 * @return Any parameters that are used to call the module
	 */
	...
	public Set<Var> getFreeVar() {
		// Only (free) variables in the parameters of a module are counted as
		// free variables.
		LinkedHashSet<Var> freeVars = new LinkedHashSet<>();
	...
	public Action applySubst(Substitution substitution) {
		...
		if (getTarget().isAnonymous()) {
			...
			parameters = new ArrayList<>(this.parameters.size());
			for (Term term : this.parameters) {
	...
	/**
	 * Returns the precondition of this {@link ModuleCallAction}. A focus
	 * action, i.e., entering a module, has the empty precondition and can
	 * always be performed (as long as its parameters are closed).
	 * <p>
	 * In case the {@link FocusMethod} of the target module is 'select' or
	 * 'filter', the condition of the parent rule of this action should be
	 * evaluated in a different style and a goal to insert into the module's
	 * attention set (goal base) should also be found. This is considered to be
	 * part of the evaluation of the rule's condition and not as part of the
	 * precondition of a focus action.
	 * </p>
	 *
	 * @param language
	 ...
	 */
	@Override
	public ModuleCallAction evaluatePrecondition(MentalState runState,
			Debugger debugger, boolean last) {
		debugger.breakpoint(Channel.CALL_MODULE, this,
				"Going to enter module: %s.", this.targetModule.getName());
		return this;
	}
